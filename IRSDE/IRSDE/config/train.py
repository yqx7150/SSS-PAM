...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        if resume_state is None:
            experiments_root = util.mkdir_and_rename(
        ...
        if opt["use_tb_logger"] and "debug" not in opt["name"]:
            ...
            tb_logger = SummaryWriter(log_dir=os.path.join(opt["path"]["experiments_root"], "log"))
    else:
        util.setup_logger(
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            current_step += 1
            print(f'current_step:\t{current_step}==========START')
            ...
            print(f'current_step:\t{current_step}==========FINISH')
