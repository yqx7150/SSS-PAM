...
class IRSDE(SDE):
    ...
    def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state', results_root=''):
        T = self.T if T < 0 else T
        x = xt.clone()
        
        state_dir = os.path.join(results_root, "state")
        os.makedirs(state_dir, exist_ok = True)
        
        for t in tqdm(reversed(range(1, T + 1))):
            score = self.score_fn(x, t)
            x = self.reverse_sde_step(x, score, t)
            tvutils.save_image(x.data, f"{state_dir}/{t}.png", normalize=False)
